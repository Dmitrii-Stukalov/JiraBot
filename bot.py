...
def jira_login(chat_id):
    ...
    bot.send_message(chat_id,
                     'Enter your Jira <a href="https://confluence.atlassian.com/cloud/api-tokens-938839638.html"> api key </a> with command /apikey "apikey"',
                     parse_mode='html')
...
def start_message(user, me):
    return "Welcome, {0}! I'm - <b>{1}</b>, bot designed to work with Jira.".format(user, me)

def create_jira_webhook(name, events, jql, server, rest_url, user, api_key):
    request = {'name': name, 'url': config.URL + '/jira', 'events': events,
               'filters': {'issue-related-events-section': jql}}
    result = requests.post(server + rest_url, json=request, auth=(user, api_key))
    print(result.request.body)
    return result.status_code

def check_webhooks(server, rest_url, user, api_key):
...
def get_hook_information(chat_id):
    ...
    bot.send_message(chat_id,
                     'Enter <a href="https://support.atlassian.com/jira-software-cloud/docs/what-is-advanced-searching-in-jira-cloud/">jql filter</a> with command /jql "YOUR FILTER"',
                     parse_mode='html')
    bot.send_message(chat_id,
                     'Enter <a href="https://developer.atlassian.com/server/jira/platform/webhooks/#configuring-a-webhook">list of events</a> that you want to track with command /events "events separated with whitespace"',
                     parse_mode='html')
...
def welcome(message):
    ...
    bot.send_message(message.chat.id,start_message(message.from_user, bot.get_me()),
...
def post_hook(message):
    create_jira_webhook(NAME, EVENTS, JQL, SERVER, REST_URL, USER, API_KEY)



@bot.message_handler(commands=['check'])
def check_hooks(message):
    check_webhooks(SERVER, REST_URL, USER, API_KEY)
